    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                patience_counter = 0
                checkpoint_path = os.path.join(checkpoint_dir, f"best_model_{datetime.now().strftime('%Y%m%d')}.pth")
                self.save_checkpoint(checkpoint_path, epoch)
                logger.info(f"Saved best model with val_acc: {val_acc:.2f}%")
            else:
                patience_counter += 1
                logger.info(f"Val_acc did not improve: {val_acc:.2f}% < best {best_val_acc:.2f}% (patience {patience_counter}/{patience})")
